...
public class DriverProvider {
    ...
    private static void updateDrivers() {
        ...
        for(int j = 0; j < nodeList.size(); j++) {
            ...
            for(int i = 0; i < nodeReq.getCapabilities().size(); i++) {
                ...
                URL driverUrl;
                try {
                    driverUrl = new URL("http://" + nodeReq.getConfigAsString("host") + ":" + nodeReq.getConfigAsString(
                            "port") + "/wd/hub");
    ...
    private static String buildDescriptionFromCapabilities(
        ...
        LOG.debug("Description of driver is: " + description);
        return description;
    }
    ...
    public static void stopDrivers() {
        for(int i = 0; i < allDrivers.size(); i++) {
            LOG.debug("Closing driver: " + allDrivers.get(i).getDescription());
            try {
                ...
                LOG.debug("For some reason a session was gone while quitting");
                LOG.debug("", e);
                continue;
            }
            catch(WebDriverException e) {
                LOG.debug("Crached webdriver, continue to closing drivers");
                continue;
            }
        }

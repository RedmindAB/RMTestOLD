...
public class DriverNamingWrapper {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(DriverNamingWrapper.class);
    private URL url;
    ...
    public WebDriver startDriver() {
        ...
        if(browser != null && this.driver == null) {
            ...
            if(this.imAFailure) {
                Assume.assumeTrue("Since driver didn't start after  " + maxRetryAttempts + " attempts, it probably wont start now ",
                        false);
                return this.driver;
            }
            else {
                ...
                while(retryAttempts <= maxRetryAttempts) {
                    try {
                        ...
                        if(capability.getCapability("rmDeviceType") == null) {
                            ...
                            LOG.debug("This is a RemoteWebDriver");
                        }
                        else {
                            ...
                            LOG.debug("This is a AppiumDriver");
                        }
                        LOG.debug("Started driver: " + description);
                    }
                    catch(UnreachableBrowserException e) {
                        LOG.debug("Having trouble starting webdriver for device: " + this.description);
                        LOG.debug("Attempt " + retryAttempts + " of " + maxRetryAttempts);
                        LOG.debug("",e);
                        retryAttempts++;
                        continue;
                    }
    ...
    private String getChromePath() {
        ...
        if(osName.startsWith("Mac")) {
            LOG.debug("Setting default chromedriver");
            return _default;
        }
        else if(osName.startsWith("Linux")) {
            LOG.debug("Setting linux chromedriver");

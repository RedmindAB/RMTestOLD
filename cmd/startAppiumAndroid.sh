...
do
	...
	$testHome/appium/bin/appium.js -U $currDevId -a $RMTestLocalNodeIp -p $basePort -bp $bootstrapPort --chromedriver-port $chromeDriverPort --nodeconfig $testHome/etc/Appium_TEMP.json  --session-override &> $logName.$currDevId.log &
	appiumStarted=true	
	while $appiumStarted
		do
